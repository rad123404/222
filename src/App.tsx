...
export default function App() {
  ...
  // Duties (2-level zones, empty by default)
  const [duties, setDuties] = useState<DutiesStore>(() => {
    const saved = localStorage.getItem('ierihon_duties');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        // Validate if legacy array format vs new 2-level zone object format
  ...
  // Birthdays (full restored list of 18 people from data_birthdays.json)
  ...
  const handleCreatePoll = () => {
    ...
    if (currentPoll && (currentPoll.voters.length > 0 || currentPoll.id)) {
      setPollHistory(prev => {
        const idx = prev.findIndex(p => p.id === currentPoll.id);
        if (idx >= 0) {
          const updated = [...prev];
          updated[idx] = { ...currentPoll };
          return updated;
        }
        return [currentPoll, ...prev];
      });
    }
  ...
  const handleVote = (status: PollStatus) => {
    ...
    setCurrentPoll(prev => {
      ...
      if (existingIdx >= 0) {
        ...
        if (oldStatus === status) return prev; // no change
      ...
      case 'settings':
        return (
          <SettingsView
            ...
            onImportSchedules={setSchedules}
            onImportHomework={setHomework}
            onImportDuties={setDuties}
            onImportBirthdays={setBirthdays}
