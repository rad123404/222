...
export const HomeworkView: React.FC<HomeworkViewProps> = ({
        ...
        {rawList.length === 0 ? (
          ...
          <div className="space-y-2.5">
            {rawList.map((item, idx) => {
              ...
              const hasHw = (homeworkStore[subjKey] || []).length > 0;
    ...
    return (
      <div className="space-y-3.5 animate-fade-in">
        ...
        <div className="grid grid-cols-2 gap-2.5">
          {SUBJECT_LIST.map(s => {
            const list = homeworkStore[s.key] || [];
  ...
  // Detail Mode
  const currentHwList = homeworkStore[activeSubjectKey] || [];
  const nextLessonISO = getSubjectNextLessonDate(activeSubjectKey);
  const nextLessonFormatted = formatCustomDate(nextLessonISO, 'day_month_long', lang);

  const handleCreateSubmit = () => {
    if (!inputText.trim()) return;
    onSaveHomework(activeSubjectKey, inputText.trim());
    setInputText('');
  ...
  const handleEditSubmit = () => {
    ...
    onEditHomework(activeSubjectKey, editModalItem.id, editText.trim());
                  ...
                  <button
                    onClick={() => {
                      if (confirm(translate('confirm_delete_hw', lang))) {
                        onDeleteHomework(activeSubjectKey, item.id);
                        haptic('success');
                      }
                    }}
